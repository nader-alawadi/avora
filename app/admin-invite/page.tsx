...
function AdminInviteContent() {
  ...
  const [inviteInfo, setInviteInfo] = useState<{ email: string; name: string | null; adminRole: string } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/auth/admin-invite/accept?token=${token}`)
      .then((r) => r.json())
      .then((d) => {
    ...
    <div className="min-h-screen gradient-hero flex items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">Join the AVORA Team</h1>
          <p className="text-white/55 mt-1 text-sm">
            You&apos;ve been invited as{" "}
            <span className="text-white font-semibold">{ROLE_LABELS[inviteInfo?.adminRole ?? ""] || inviteInfo?.adminRole}</span>
          </p>
        </div>

        <div className="glass rounded-2xl shadow-xl p-8">
          <div className="bg-white/10 rounded-xl p-3 mb-6 text-center">
