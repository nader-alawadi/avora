...
interface BusboyOpts { headers: Record<string, string>; limits?: { fileSize?: number } }
  ...
  return new Promise<{ buffer: Buffer; filename: string; mimeType: string }>(
    (resolve, reject) => {
      ...
      bb.on("file", (field, stream, info) => {
        ...
        stream.on("end", () => {
          if ((stream as { truncated?: boolean }).truncated) { reject(new Error("FILE_TOO_LARGE")); }
          else { resolved = true; resolve({ buffer: Buffer.concat(chunks), filename: info.filename, mimeType: info.mimeType }); }
        });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (msg === "FILE_TOO_LARGE") return NextResponse.json({ error: "File exceeds 20 MB limit" }, { status: 400 });
  ...
  const safeName = `${session.id}-${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
  const filePath = path.join(UPLOAD_DIR, safeName);

  try { await writeFile(filePath, buffer); } catch (e) {
    return NextResponse.json({ error: "Failed to save file", detail: String(e) }, { status: 500 });
  }

  return NextResponse.json({ fileUrl: `/uploads/onboarding/${safeName}`, filename, mimeType });
}
