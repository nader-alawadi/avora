...
function enSummary(s: { summary?: string; englishSummary?: string } | null | undefined): string {
  if (!s) return "";
  if (s.englishSummary) return s.englishSummary;
  return ls(s.summary);
}
...
// ─── Design tokens (4-pt grid) ────────────────────────────────────────────
// sp4=4  sp8=8  sp12=12  sp16=16  sp20=20  sp24=24  sp40=40

const S = StyleSheet.create({
  // ── Pages ─────────────────────────────────────────────────────────────────
  page:      { backgroundColor: WHITE, fontFamily: "Helvetica" },
  coverPage: { backgroundColor: TEAL,  fontFamily: "Helvetica", padding: 48 },
  // 40pt horizontal margins match the section bar; 24pt top gap after bar;
  // 60pt bottom so content never bleeds under the 20pt-from-bottom footer.
  content:   { paddingHorizontal: 40, paddingTop: 24, paddingBottom: 60 },

  // ── Cover ─────────────────────────────────────────────────────────────────
  coverLogo:      { fontSize: 42, fontFamily: "Helvetica-Bold", color: WHITE, letterSpacing: 3 },
  coverTagline:   { fontSize: 14, color: WHITE, fontFamily: "Helvetica", marginTop: 8 },
  coverByline:    { fontSize: 10, color: WHITE, fontFamily: "Helvetica", marginTop: 4 },
  coverAccent:    { height: 3, backgroundColor: CORAL, marginTop: 40, marginBottom: 32, width: 56 },
  coverTitle:     { fontSize: 20, fontFamily: "Helvetica-Bold", color: WHITE, marginBottom: 24 },
  // flexWrap + gap is unreliable in some react-pdf builds; use explicit
  // marginRight / marginBottom on each box instead.
  coverGrid:      { flexDirection: "row", flexWrap: "wrap" },
  coverMetaBox:   { backgroundColor: "rgba(255,255,255,0.13)", borderRadius: 8, padding: 16, width: "47%", marginRight: "3%", marginBottom: 12 },
  coverMetaLbl:   { fontSize: 8,  color: WHITE, fontFamily: "Helvetica",      marginBottom: 4 },
  coverMetaVal:   { fontSize: 15, color: WHITE, fontFamily: "Helvetica-Bold" },
  coverFooter:    { position: "absolute", bottom: 28, left: 48, right: 48 },
  coverFooterTxt: { fontSize: 8, color: WHITE, fontFamily: "Helvetica" },

  // ── Section header bar ────────────────────────────────────────────────────
  secBar:      { backgroundColor: TEAL, paddingHorizontal: 40, paddingVertical: 20 },
  secBarTitle: { fontSize: 18, fontFamily: "Helvetica-Bold", color: WHITE },
  secBarSub:   { fontSize: 9,  color: WHITE, fontFamily: "Helvetica", marginTop: 4 },

  // ── Page footer (fixed — renders on every page) ───────────────────────────
  footer: {
  ...
  // subHeading (13pt) → cardTitle (11pt) → body (10pt)
  subHeading: { fontSize: 13, fontFamily: "Helvetica-Bold", color: DARK, marginTop: 20, marginBottom: 10 },
  label:      { fontSize: 8,  fontFamily: "Helvetica-Bold", color: GRAY,  marginBottom: 6, letterSpacing: 0.5 },
  body:       { fontSize: 10, color: DARK, lineHeight: 1.6 },
  note:       { fontSize: 9,  color: GRAY, lineHeight: 1.5, fontFamily: "Helvetica-Oblique" },

  // ── Summary card ─────────────────────────────────────────────────────────
  summaryCard: {
    backgroundColor: TEAL_LT, borderLeftWidth: 3, borderLeftColor: TEAL,
    paddingHorizontal: 14, paddingVertical: 12, marginBottom: 16, borderRadius: 3,
  },
  summaryLbl: { fontSize: 8, fontFamily: "Helvetica-Bold", color: TEAL, marginBottom: 5 },
  summaryTxt: { fontSize: 10, color: DARK, lineHeight: 1.6 },

  // ── Generic card ─────────────────────────────────────────────────────────
  card:      { backgroundColor: GRAY_LT, borderRadius: 6, padding: 14, marginBottom: 12 },
  cardTitle: { fontSize: 11, fontFamily: "Helvetica-Bold", color: DARK, marginBottom: 8 },

  // ── Tags / pills ─────────────────────────────────────────────────────────
  // Each tag carries its own marginRight + marginBottom so flexWrap works
  // reliably without depending on the `gap` property.
  tagRow:     { flexDirection: "row", flexWrap: "wrap" },
  tagTeal:    { backgroundColor: TEAL_LT, borderRadius: 10, paddingHorizontal: 9, paddingVertical: 4, marginRight: 5, marginBottom: 5 },
  tagTealTxt: { fontSize: 8, color: TEAL },
  tagGray:    { backgroundColor: "#E5E7EB", borderRadius: 10, paddingHorizontal: 9, paddingVertical: 4, marginRight: 5, marginBottom: 5 },
  tagGrayTxt: { fontSize: 8, color: "#374151" },
  tagDark:    { backgroundColor: "#1F2A2A",  borderRadius: 10, paddingHorizontal: 9, paddingVertical: 4, marginRight: 5, marginBottom: 5 },
  tagDarkTxt: { fontSize: 8, color: "#A3E4D7" },

  // ── Bullet list ──────────────────────────────────────────────────────────
  // Marker has a fixed 14pt width + 4pt right margin = 18pt indent for the text.
  bulletRow:     { flexDirection: "row", marginBottom: 6 },
  bulletMark:    { fontSize: 10, color: TEAL,  width: 14, marginRight: 4, flexShrink: 0, fontFamily: "Helvetica-Bold" },
  bulletMarkRed: { fontSize: 10, color: CORAL, width: 14, marginRight: 4, flexShrink: 0, fontFamily: "Helvetica-Bold" },
  bulletTxt:     { fontSize: 10, color: DARK, flex: 1, lineHeight: 1.5 },

  // ── Key-value pair ────────────────────────────────────────────────────────
  kvRow: { flexDirection: "row", marginBottom: 8 },
  kvLbl: { fontSize: 9, fontFamily: "Helvetica-Bold", color: GRAY, width: 96, marginRight: 8, flexShrink: 0 },
  kvVal: { fontSize: 9, color: DARK, flex: 1, lineHeight: 1.5 },

  // ── Two-column layout ─────────────────────────────────────────────────────
  // Use colL for the left column (adds a 12pt right gap) and col2 for the
  // right column.  Never rely on `gap` on the parent row.
  row:  { flexDirection: "row" },
  colL: { flex: 1, marginRight: 12 },
  col2: { flex: 1 },

  // ── Code / template box ───────────────────────────────────────────────────
  codeBox: { backgroundColor: "#1F2A2A", borderRadius: 5, padding: 12, marginBottom: 8 },
  codeTxt: { fontSize: 9, color: "#6EE7B7", fontFamily: "Helvetica", lineHeight: 1.5 },

  // ── Messaging framework 3-up ──────────────────────────────────────────────
  mfRow: { flexDirection: "row", marginBottom: 10 },
  mfBox: { flex: 1, borderRadius: 5, padding: 10 },
  mfLbl: { fontSize: 8, fontFamily: "Helvetica-Bold", marginBottom: 4 },
  mfTxt: { fontSize: 9, lineHeight: 1.5 },

  // ── Tier card ─────────────────────────────────────────────────────────────
  tierCard:     { borderWidth: 1, borderColor: DIVIDER, borderRadius: 6, padding: 14, marginBottom: 12 },
  ...
  tierCount:    { fontSize: 9, color: GRAY },
  ...
  return (
    <Page size="A4" style={S.coverPage}>
      ...
      <View style={S.coverFooter}>
        <Text style={S.coverFooterTxt}>
          Generated {date}  |  Confidential — Enigma Sales / AVORA Platform
        </Text>
      </View>
    </Page>
  );
    ...
    <>
      <Page size="A4" style={S.page}>
        ...
        <View style={S.content}>
          ...
          {warnings.length > 0 && (
            <View style={{ backgroundColor: "#FFFBEB", borderLeftWidth: 3, borderLeftColor: "#F59E0B", paddingHorizontal: 14, paddingVertical: 12, borderRadius: 3, marginBottom: 16 }}>
              <Text style={{ fontSize: 9, fontFamily: "Helvetica-Bold", color: "#92400E", marginBottom: 6 }}>DATA WARNINGS</Text>
                ...
                {priorities.map((pr, i) => (
                  <View
                    ...
                    style={{
                      ...
                      borderRadius: 5,
                      padding: 10,
                      marginRight: i < priorities.length - 1 ? 8 : 0,
                    }}
                  >
                    <Text style={{ fontSize: 9, color: TEAL, lineHeight: 1.5 }}>{pr}</Text>
                  </View>
                ))}
              </View>
            </View>
      ...
      {(triggers.length > 0 || qualifiers.length > 0 || disqualifiers.length > 0) && (
        <Page size="A4" style={S.page}>
          ...
          <View style={S.content}>
            <View style={S.row}>
              {triggers.length > 0 && (
                <View style={qualifiers.length > 0 ? S.colL : S.col2}>
          ...
          {roles.map((role, i) => {
            ...
            return (
              <View key={i} style={S.card}>
                {/* Role badge + decision weight */}
                <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                  <View style={{ backgroundColor: accent.bg, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 4 }}>
                    <Text style={{ fontSize: 9, fontFamily: "Helvetica-Bold", color: accent.txt }}>{roleName}</Text>
                  </View>
                  {weight && <Text style={S.note}>{weight}</Text>}
                </View>

                {/* Titles + concerns on left, messaging angle on right */}
                <View style={S.row}>
                  <View style={S.colL}>
                    {titles.length > 0 && (
                  ...
                  {angle && (
                    <View style={S.col2}>
                      <View style={{ backgroundColor: TEAL_LT, borderRadius: 5, padding: 12 }}>
                        <Text style={{ fontSize: 8, fontFamily: "Helvetica-Bold", color: TEAL, marginBottom: 5 }}>
                          MESSAGING ANGLE
                        </Text>
                        <Text style={{ fontSize: 9, color: DARK, lineHeight: 1.5 }}>{angle}</Text>
                      </View>
                    </View>
                  )}
                </View>
              ...
              return (
                ...
                >
                  <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <View style={{ backgroundColor: style.badge.bg, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 4 }}>
                      <Text style={{ fontSize: 9, fontFamily: "Helvetica-Bold", color: style.badge.txt }}>
                        {ls(tier.tier)}
                      </Text>
                    </View>
                  ...
                  {ls(tier.approach) && (
                    <Text style={{ fontSize: 9, color: DARK, lineHeight: 1.5 }}>{ls(tier.approach)}</Text>
                  )}
                </View>
              );
            })}
    ...
    <Page size="A4" style={S.page}>
      ...
      <View style={S.content}>
        ...
        {ls(out.sequenceOverview) && (
          <View style={{ ...S.card, borderLeftWidth: 3, borderLeftColor: TEAL, marginBottom: 16 }}>
            <Text style={{ ...S.label, marginBottom: 8 }}>SEQUENCE OVERVIEW</Text>
        ...
        {channels.map((ch, i) => {
          ...
          const hasHook   = !!ls(mf.hook);
          const hasValue  = !!ls(mf.value);
          const hasCta    = !!ls(mf.cta);

          return (
            <View key={i} style={{ marginBottom: 20 }}>
              {/* Channel header: name badge + cadence pill */}
              <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 10 }}>
                <View style={{ backgroundColor: TEAL, borderRadius: 4, paddingHorizontal: 10, paddingVertical: 5, marginRight: 8 }}>
                  <Text style={{ fontSize: 10, fontFamily: "Helvetica-Bold", color: WHITE }}>{ls(ch.channel)}</Text>
                </View>
                {ls(ch.cadence) && (
                  <View style={{ backgroundColor: GRAY_LT, borderRadius: 4, paddingHorizontal: 8, paddingVertical: 5 }}>
              ...
              {ls(ch.strategy) && (
                <Text style={{ ...S.note, marginBottom: 10 }}>{ls(ch.strategy)}</Text>
              )}

              {/* Messaging framework — Hook / Value / CTA */}
              {(hasHook || hasValue || hasCta) && (
                <View style={S.mfRow}>
                  {hasHook && (
                    <View style={{ ...S.mfBox, backgroundColor: "#FFFBEB", marginRight: 8 }}>
                      <Text style={{ ...S.mfLbl, color: "#92400E" }}>HOOK</Text>
                      <Text style={{ ...S.mfTxt, color: DARK }}>{ls(mf.hook)}</Text>
                    </View>
                  )}
                  {hasValue && (
                    <View style={{ ...S.mfBox, backgroundColor: "#F0FDF4", marginRight: hasCta ? 8 : 0 }}>
...
function LookalikePages({ data }: { data: Record<string, unknown> | null }) {
  ...
  const linkedinQueries  = lsArr(sq.linkedin);
  const googleQueries    = lsArr(sq.google);
  const crunchbaseFilters = lsArr(sq.crunchbase);
  const hasQueries = linkedinQueries.length > 0 || googleQueries.length > 0 || crunchbaseFilters.length > 0;
  ...
  return (
    <Page size="A4" style={S.page}>
      ...
      <View style={S.content}>
        ...
        <View style={{ backgroundColor: "#EFF6FF", borderLeftWidth: 3, borderLeftColor: "#2563EB", paddingHorizontal: 14, paddingVertical: 12, borderRadius: 3, marginBottom: 16 }}>
          <Text style={{ fontSize: 8, fontFamily: "Helvetica-Bold", color: "#1E40AF", marginBottom: 5 }}>
            SEARCH CRITERIA ONLY — NO PERSONAL DATA
          </Text>
        ...
        {cats.length > 0 && (
          <>
            ...
            <View style={S.row}>
              {cats.map((cat, i) => (
                <View key={i} style={i % 2 === 0 && i < cats.length - 1 ? S.colL : S.col2}>
                  <View style={S.card}>
                    <Text style={S.cardTitle}>{ls(cat.category)}</Text>
                    {ls(cat.rationale) && (
                      <Text style={{ ...S.note, marginBottom: 10 }}>{ls(cat.rationale)}</Text>
                    )}
        ...
        {hasQueries && (
          <>
            ...
            {crunchbaseFilters.length > 0 && (
              <View style={{ marginBottom: 12 }}>
                <Text style={S.label}>CRUNCHBASE FILTERS</Text>
                {crunchbaseFilters.map((q, i) => (
                  <View key={i} style={{ backgroundColor: GRAY_LT, borderRadius: 5, padding: 10, marginBottom: 6 }}>
                    <Text style={{ fontSize: 9, color: DARK, lineHeight: 1.5 }}>{q}</Text>
                  </View>
                ))}
              </View>
            )}
      ...
      <View style={S.content}>
        {steps.map((step, i) => (
          <View key={i} style={{ flexDirection: "row", alignItems: "flex-start", marginBottom: 20 }}>
            {/* Numbered circle */}
            <View style={{ width: 28, height: 28, backgroundColor: TEAL, borderRadius: 14, alignItems: "center", justifyContent: "center", flexShrink: 0, marginRight: 16, marginTop: 1 }}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ fontSize: 10, color: GRAY, lineHeight: 1.6 }}>{step.desc}</Text>
            </View>
          </View>
        ))}

        <View style={{ marginTop: 28, backgroundColor: TEAL_LT, borderRadius: 8, padding: 16, borderWidth: 1, borderColor: "#B2D8D4" }}>
          <Text style={{ fontSize: 11, fontFamily: "Helvetica-Bold", color: TEAL, marginBottom: 8 }}>About this Report</Text>
          <Text style={{ fontSize: 9, color: DARK, lineHeight: 1.6 }}>
